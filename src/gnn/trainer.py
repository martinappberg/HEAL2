...
class Trainer:
    ...
    def train_and_test(self, model, ggi_graph, loss_fn, optimizer, metric_funcs, train_loader, val_loader, test_loader=None, evaltrain_loader=None):
        ...
        for cur_step in range(len(train_loader)):
            ...
            if (cur_step+1) % self.eval_interval == 0:
                ...
                val_scores, val_attn_list, val_predictions, val_z_sae_list, val_feature_importance = self.evaluate(model, ggi_graph, val_loader, metric_funcs)
                if val_scores['auroc'] > best_val_scores['auroc']:
                    ...
                    if test_loader is not None:
                        best_test_scores, best_test_attn_list, test_predictions, test_z_sae_list, test_feature_importance = self.evaluate(model, ggi_graph, test_loader, metric_funcs)
                    if evaltrain_loader is not None:
                        best_train_scores, best_train_attn_list, train_predictions, train_z_sae_list, train_feature_importance = self.evaluate(model, ggi_graph, evaltrain_loader, metric_funcs)
                    cur_early_stop = 0
                else:
                    cur_early_stop += 1
    ...
    def evaluate(self, model, ggi_graph, test_loader, metric_funcs):
        ...
        for batch in test_loader:
            ...
            labels, preds, attn_scores, sample_ids, _, _ = self.forward_batch(model, ggi_graph, batch)

            # Compute gradients for feature importance by summing over the predictions
            output_sum = preds.sum()
            grads = torch.autograd.grad(outputs=output_sum, inputs=feats, retain_graph=False)[0]
            grads = grads.view(batch_size, num_nodes, -1) # [batch_size, num_nodes, num_features]

            # Now, calculate feature importance for each sample in the batch
            for i in range(batch_size):
                # Extract gradients for the current sample
                sample_grads = grads[i]  # Shape should be [num_nodes, num_features]
